...
Um einen Entscheidungsbaum generieren zu können gibt es verschiedene Algorithmen und Ansätze die im folgenden Abschnitt kurz erklärt werden sowie die von RapidMiner verwendeten hervorgehoben werden. Die betrachteten Algorithmen sind:
\begin{pitemize}
\item ID3
\item C4.5
\item CART
...
\textbf{ID3} \\
Der ID3 Algorithmus (Iterative Dichotomiser 3) wurde von John Ross Quinlan in 1986 entwickelt und ist auch der Vorgänger C4.5. ID3 wurde entworfen um nicht alle möglichen Entscheidungsbäume generieren zu müssen, wenn man nur einen möglichst guten Entscheidungsbaum für die gegebenen Daten benötigt. Im Allgemeinen generiert ID3 einfache Entscheidungsbäume aber es kann nicht garantiert werden, dass es nicht bessere Entscheidungsbäume geben würde. Der allgemeine Algorithmus ist iterativ und geht dabei wie folgt vor: \cite{john_ross_quinlan_1986}

\begin{pitemize}
\item Es wird eine Teilmenge der Trainingsdaten per Zufall ausgewählt (Window) und für dieses ein Entscheidungsbaum generiert. Dieser Entscheidungsbaum ist für alle Elemente innerhalb der Teilmenge gültig und kann diese korrekt identifizieren. 
\item Jetzt werden alle anderen Elemente mit diesem Entscheidungsbaum klassifiziert. Kann der Baum alle Elemente korrekt klassifizieren, so ist ein ausreichend guter Entscheidungsbaum gefunden und das Ziel erreicht. Wenn nicht alle Elemente korrekt klassifiziert werden können, wird eine Auswahl der falsch klassifizierten Elemente in die Teilmenge (Window) übernommen und erneut ein Entscheidungsbaum gesucht. 
\end{pitemize}

Für das Erstellen des Entscheidungsbaumes selbst wird dabei auf das finden des Attributs mit der niedersten Entropie für den aktuellen Knoten gesetzt. Entropie ist in diesem Zusammenhang ein Wert der die Höhe der Verunreinigung angibt. \cite{howard_hamilton_machine_2009} Dies bedeutet, man sucht einen Attribut, welches für eine möglichst reine Menge an Objekten in diesem Knoten bewirkt. Hat man zum Beispiel eine Menge von männlichen und weiblichen Personen so sucht man ein Attribut, welches bewirkt, dass im nächsten Knoten eine möglichst hohe Wahrscheinlichkeit für männlich oder weiblich gegeben ist und somit die Anzahl nicht ausbalanziert ist. Wäre die Anzahl genau ausbalanciert, so würde man durch diesen Knoten keinen Schritt näher an die schlussendliche Klassifizierung kommen.
...
\item Handhabung von nicht diskreten Werten durch Aufteilen der Objekte bei denen das Attribut  größer oder einen kleiner gleich ist.
